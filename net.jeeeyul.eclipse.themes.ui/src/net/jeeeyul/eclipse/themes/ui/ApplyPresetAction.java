...
public class ApplyPresetAction extends Action {
	...
	public ApplyPresetAction(IJTPreset preset) {
		 ...
		 * getThemeEngine().getActiveTheme()이 null인 경우가 있다?
		 */
		if (getThemeEngine().getActiveTheme() != null && getThemeEngine().getActiveTheme().getId().equals(JThemesCore.CUSTOM_THEME_ID)) {
	...
	public void run() {
		...
		IJTPreset defaultPreset = presetManager.getDefaultPreset();
		Properties properties;
		if(defaultPreset == null) {
			properties = PropertiesUtil.merge(preset.getProperties());
		}else {
			properties = PropertiesUtil.merge(presetManager.getDefaultPreset().getProperties(), preset.getProperties());
		}
