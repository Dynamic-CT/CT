	...
	protected Control createContents(Composite parent) {
		...
		viewer.setContentProvider(new IStructuredContentProvider() {
			...
			public Object[] getElements(Object inputElement) {
				...
				return JThemesCore.getDefault().getPresetManager().getDefaultCategory().getPresets().toArray();
			}
...
//		viewer.setInput(JThemesCore.getDefault().getPresetManager().getUserCategory());
		viewer.setInput(JThemesCore.getDefault().getPresetManager().getDefaultCategory());
		...
		activateButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				// TODO: action 이전에 재실행 여부 로직 추가, 재실행 여부 '예' 이면 
				// active() + iconChange로직 이후 재시작 로직 태우기
				activate();
			}
		});
